...
client.on("message", async (message) => {
    ...
    if (message.content.startsWith("!join")) {
        if (!message.member.voice.channel) return message.channel.send("Join the voice channel first.")
        if(!message.content.substring(6)) return message.channel.send("Please enter a song url or query to search.")

        await client.Audio.connect(message.member.voice.channel)

        return message.channel.send(`I Connected to ${message.member.voice.channel}`)
    }

    if (message.content.startsWith("!play")) {
        const VC = codo.getVC(message.guild.id, true)

        if (!VC) return message.channel.send("Please type `!join` first.")
